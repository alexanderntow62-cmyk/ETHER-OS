...
import '../system/ether_system_action_engine.dart';
...
class EtherBrain {
  ...
  Future<String> think(String message) async {
    ...
    final intent = intents.classify(input);
    ...
    if (greetingMatch) {
      final response =
          'Hello 👋. I am $name, the intelligence core of ETHER-OS. My brain is online and ready to help.';
      memory.rememberEther(response);
      return response;
    }
    ...
    if (intent.isSystemAction) {
      ...
      memory.rememberEther(systemResult);
      return systemResult;
    }

    // Try registered skills before normal conversation.
    final skillResult = intent.isSkill ? await skills.tryHandle(input) : null;
